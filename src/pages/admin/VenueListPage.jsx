...
import { getVenues, deleteVenue, updateVenue } from '../../services/adminService'
...
function VenueListPage() {
  ...
  const fetchVenues = async () => {
    setLoading(true)
    try {
      const data = await getVenues(page, PAGE_SIZE)
  ...
  const handleToggleActive = async (venue) => {
    try {
      ...
      addToast(`Venue ${venue.is_active ? 'deactivated' : 'activated'} successfully`)
      fetchVenues()
    } catch (err) {
      const errorMsg = err.response?.data?.message || err.response?.data?.detail || 'Failed to update venue status'
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true)
    try {
      await deleteVenue(deleteModal.item.id)
      setDeleteModal({ open: false, item: null })
      addToast('Venue deleted successfully')
      fetchVenues()
    } catch (err) {
      setDeleteModal({ open: false, item: null })
      const errorMsg = err.response?.data?.message || err.response?.data?.detail || 'Failed to delete venue'
      addToast(errorMsg, 'error')
    } finally {
      setDeleting(false)
    }
  ...
  const columns = [
    ...
    { key: 'contact', label: 'Contact', render: (val) => val || '-' },
    {
      key: 'screens',
      label: 'Screens',
      render: (val) => (
        <span className="px-2 py-1 bg-gray-100 rounded-full text-xs font-medium">
          {val?.length || 0} screens
        </span>
      )
    },
    {
  ...
  if (loading) {
    return (
      <AdminLayout>
        <Spinner.Page message="Loading venues..." />
      </AdminLayout>
    )
  }
  ...
  return (
    <AdminLayout>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Venues</h1>
        <Link
          to="/admin/venues/new"
      ...
      <DataTable
        ...
        data={venues}
        actions={actions}
        emptyMessage="No venues found. Create your first venue!"
        page={page}
        pageSize={PAGE_SIZE}
        totalCount={totalCount}
        onPageChange={handlePageChange}
        onRowClick={(row) => `/admin/venues/${row.id}`}
      />
      ...
      <DeleteConfirmModal
        ...
        loading={deleting}
        title="Delete Venue"
        message={`Are you sure you want to delete "${deleteModal.item?.name}"? This will also delete all screens and shows.`}
      />
    </AdminLayout>
  )
}
